;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         edus Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('edus_about_me_two') != 1){
                prefetch();
            }
        });

        $('body').off('click','.edus_flogo_uploader');

        $('body').on('click','.edus_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.edus-logo-preview');
                ...
                $(el).siblings().children('.edus_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.edus_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.edus_logo_id').val());
                var atts = $(el).siblings().children('.edus_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.edus_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.edus-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.edus_flogo_uploader').val('Change Image');
