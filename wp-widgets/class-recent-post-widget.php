...
class Edus_Recent_Post_Widget extends WP_Widget
{
	...
	public function __construct()
	{
		parent::__construct(
			'edus_popular_posts',
			esc_html__('Edus: Recent Posts with Thumbnails', 'edus-master'),
			array('description' => esc_html__('Display your recent posts, with a thumbnail.', 'edus-master'))
		);
	}
	...
	public function widget($args, $instance)
	{
		...
		if ($recent_articles->have_posts()) : ?>
			<ul class="recent_post_item">
				...
				esc_html__(' Oops, there are no posts.', 'edus-master')
				?>
	...
	public function form($instance)
	{
		...
		if (!empty($instance) && $instance['title']) {
			...
			$title = esc_html__('Recent Posts', 'edus-master');
		}
		...
		$orderby_arr = array(
			...
			'comment_count' => esc_html__('Most Comment', 'edus-master')
		);
		...
		?>
			...
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('no_of_posts')) ?>"><?php esc_html_e('No Of Posts', 'edus-master'); ?></label>
...
if (!function_exists('Edus_Recent_Post_Widget')) {
	function Edus_Recent_Post_Widget()
	{
		register_widget('Edus_Recent_Post_Widget');
	}

	add_action('widgets_init', 'Edus_Recent_Post_Widget');
}
